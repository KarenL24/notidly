...
export default function ReviewPage() {
  ...
  // Get title, file, pathname, and musicxml from query params
  const pieceTitle = searchParams.get("title") || "Untitled"
  const fileName = searchParams.get("file") || "audio.mp3"
  const pathname = searchParams.get("pathname") || ""
  const musicxml = searchParams.get("musicxml") 
    ? decodeURIComponent(searchParams.get("musicxml")!) 
    : null
  
  // Source info from transcription
  const tempoBpm = searchParams.get("tempoBpm") || "0"
  const source = searchParams.get("source") || "UNKNOWN"
  const isPythonBackend = searchParams.get("isPythonBackend") === "true"
  const warnings = searchParams.get("warnings") 
    ? JSON.parse(searchParams.get("warnings")!) 
    : []
